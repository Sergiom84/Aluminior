 ...
 *
 * v5: el aprendizaje del multiplicador '!' incorpora rasgos nuevos de la
 * instancia — recuentos por FUNCIÓN (fn:HV = nº de piezas de perfil de
 * hoja…) y por artículo GENÉRICO (gen:97 = nº de travesaños grandes…) —
 * para que "junta = una por pieza de hoja" (S.7.2, delta 0) o "taco = uno
 * por travesaño" se aprendan solos con los umbrales de siempre.
 *
 * Solo lectura. Uso: npx tsx scripts/medir-seleccion-v5.mjs
 */
...
const rasgosInstancia = new Map() // tipo|doc|linea -> Map(rasgo -> n)  fn:/gen:
...
const multiplicador = new Map() // texto -> {rasgo, k}
for (const [texto, obs] of obsCategoria) {
  ...
  for (const nombre of nombres) {
    // constante k aprendida: moda de real/(base×rasgo)
    ...
    // preferir k=1 en empates (menos parámetros)
